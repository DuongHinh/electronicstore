...
.directive('amMultiselect', ['$parse', '$document', '$compile', '$interpolate', 'optionParser',
    ...
    return {
        ...
        isCloseMultiple = attrs.closeMultiple || false,
        isMultiple = isCloseMultiple || (attrs.multiple || false),
        required = false,
        ...
        // watch single/multiple state for dynamically change single to multiple
        scope.$watch(function () {
            return $parse(attrs.closeMultiple)(originalScope);
        }, function (newVal) {
            isCloseMultiple = newVal || false;
            isMultiple = isCloseMultiple || (attrs.multiple || false);
        });
        ...
        var msClickCallback;
        scope.$watch(function () {
            return attrs.msClick;
        }, function (newVal) {
            msClickCallback = $parse(newVal);
        }, true);
        ...
        function is_empty(obj) {
            ...
            // added check if obj is array skip prop existing check
        ...
        function selectCloseMultiple(curritem) {
            curritem.checked = !curritem.checked;

            var indexArr = scope.items.map(function (val, index) { if (val.checked) return index; });
            var minCheckedIndex = Math.min.apply(null, indexArr.filter(function (val) { return val != undefined }));
            var maxCheckedIndex = Math.max.apply(null, indexArr.filter(function (val) { return val != undefined }));

            if (curritem.checked) {
                for (var i = minCheckedIndex; i <= maxCheckedIndex; i++) {
                    scope.items[i].checked = true;
                }
            } else {
                if (maxCheckedIndex >= scope.items.indexOf(curritem) && scope.items.indexOf(curritem) >= minCheckedIndex) {
                    for (var i = maxCheckedIndex; i >= minCheckedIndex; i--) {
                        if (scope.items[i] == curritem) break;
        ...
        scope.select = function (item) {
            if (isMultiple === false) {
                ...
                if (isCloseMultiple === false) {
                    selectMultiple(item);
                } else {
                    selectCloseMultiple(item);
                }
            }
            msClickCallback(originalScope, { $item: item });
        }
        }
    };
}])
